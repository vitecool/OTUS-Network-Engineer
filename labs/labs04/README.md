...
ip route 10.16.2.0 255.255.255.0 10.16.4.1 50 track 1 (указываем более высокую метрику 50 и вешаем track 1,т.е. когда 10.16.4.1 доступен маршрут появляется в таблице маршрутизации и перекрывает более низкий,если ляжет он пропадет из таблицы)

ip route 10.16.2.0 255.255.255.0 10.16.5.1 100  (указываем более низкий приоритет метрики 100)
Аналогично настроен ip sla для VRRP в Санкт - Петербурге